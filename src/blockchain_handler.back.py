...
def deploy_contracts():
    ...
    address_DAO = deploy_contract(contracts[0])
    address_wallet = deploy_contract(contracts[1])

    addresses = [address_DAO, address_wallet]
...
# Deposits ETH into the EvilHackerWallet contract. Transaction is signed by the bot
def deposit_to_wallet(value: int):
    contract = 'EvilHackerWallet'

    EvilHackerWallet = get_w3_contract(contract)
    tx_hash = EvilHackerWallet.functions.deposit().transact({'from': PUB_KEY_BOT, 'value': value * 10**18})
